...
const checkOption = (group, value, label, checked) => `
  <li class="filter__option">
    <label class="filter-check">
      <input class="filter-check__input" type="checkbox" data-filter-value name="${group}" value="${value}"${checked ? ' checked' : ''} />
      <span class="filter-check__box">
        <svg class="filter-check__icon filter-check__icon--off" aria-hidden="true"><use href="#icon-radio"></use></svg>
        <svg class="filter-check__icon filter-check__icon--on" aria-hidden="true"><use href="#icon-radio-checked"></use></svg>
      </span>
...
function chip(key, label, count, active) {
  ...
  return `
    <button class="filter-chip${active ? ' filter-chip--active' : ''}" type="button" data-filter-toggle="${key}" aria-haspopup="true" aria-expanded="false">
      ${label}
      ${marker}
    </button>`;
}
...
function checkboxFilter(key, label, options, applied, formatValue = (v) => v) {
  ...
  return `
    <div class="filter" data-filter="${key}">
      ${chip(key, label, applied.length, active)}
...
function render() {
  ...
  if (count) count.textContent = data.total ? `${data.total} ${plural(data.total, ['товар', 'товара', 'товаров'])}` : '';
