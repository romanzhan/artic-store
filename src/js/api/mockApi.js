...
const PROMOS = {
  SALE: { code: 'SALE', type: 'fixed', value: 2500 },
  ARTIC15: { code: 'ARTIC15', type: 'percent', value: 15 },
};

const BRANCHES = [
  { id: 'artic', name: 'Artic Store', address: 'ул. Белинского, 8', hours: 'Ежедневно с 10:00 до 22:00', pickup: '16 мая', image: 'branch-1', coords: [56.0106, 92.8526] },
  { id: 'branch-2', name: 'Филиал 2', address: 'ул. Белинского, 8', hours: 'Ежедневно с 10:00 до 22:00', pickup: '17 мая', image: 'branch-2', coords: [56.0301, 92.9089] },
];

const ORDERS = [
  {
    id: 256525, date: '25 июня 2026', time: '10:00 - 18:00',
    delivery: 'Самовывоз', address: 'г. Красноярск, ул. Белинского, 8', payment: 'Наличный',
    status: 'created', discount: 2500, shipping: 1500,
    items: [{ id: 5, size: 38, color: 'Зелёный', qty: 1 }, { id: 7, size: 38, color: 'Белый', qty: 2 }],
  },
  {
    id: 256523, date: '22 июня 2026', time: '12:00 - 19:00',
    delivery: 'Адрес доставки', address: 'г. Красноярск, ул. Ленина, 126, кв. 4', payment: 'Онлайн',
    status: 'done', discount: 0, shipping: 0,
    items: [{ id: 9, size: 44, color: 'Чёрный', qty: 1 }, { id: 3, size: 'M', color: 'Бежевый', qty: 1 }],
  },
  {
    id: 256521, date: '18 июня 2026', time: '12:00 - 19:00',
    delivery: 'Адрес доставки', address: 'г. Красноярск, пр. Мира, 10, кв. 22', payment: 'Безналичный',
...
export const mockApi = {
  ...
  getOrders() {
    return structuredClone(
      ORDERS.map((order) => {
        ...
        return { ...order, items, itemsTotal, total: itemsTotal - order.discount + order.shipping };
      }),
    );
  },
  ...
  findPromo(code) {
    ...
    return promo ? structuredClone(promo) : null;
  },
  ...
  async getGiftCard() {
    ...
    return structuredClone({
      ...
      amounts: [10000, 15000, 25000, 50000, 100000],
      types: [
        { id: 'envelope', label: 'В подарочном конверте', icon: 'icon-gift-envelope' },
        { id: 'digital', label: 'В электронном виде', icon: 'icon-gift-digital' },
      ],
    });
  },
  ...
  getCheckout() {
    return structuredClone({
      deliveryMethods: [
        { id: 'delivery', icon: 'icon-delivery', title: 'Доставка', desc: 'Доставка до адреса бесплатно от 15 000 ₽' },
        { id: 'pickup', icon: 'icon-pickup', title: 'Самовывоз', desc: 'Бесплатно, примерка в нашем уютном магазине' },
      ],
      paymentMethods: [
        { id: 'cash', icon: 'icon-cash', title: 'Наличный' },
        { id: 'online', icon: 'icon-pay-online', title: 'Оплата онлайн' },
        { id: 'cashless', icon: 'icon-cashless', title: 'Безналичный' },
      ],
      address: 'г. Красноярск, ул. Красной Гвардии, 21, кв. 26',
      dates: [
        { id: '22', day: '22 янв', weekday: 'чт' },
        { id: '23', day: '23 янв', weekday: 'пт' },
        { id: '24', day: '24 янв', weekday: 'сб' },
        { id: '25', day: '25 янв', weekday: 'вс' },
        { id: '26', day: '26 янв', weekday: 'пн' },
      ],
      times: ['12:00 - 19:00', '19:00 - 22:00'],
      reserveDays: 3,
      branches: BRANCHES,
    });
  },
  ...
  sendContact(payload) {
    return structuredClone({ ok: true, ...payload });
  },
  ...
  searchAddress(query) {
    ...
    const pool = [
      'г. Красноярск, ул. Красной Армии',
      'г. Красноярск, ул. Красной Гвардии',
      'г. Красноярск, ул. Красноярский рабочий',
      'г. Красноярск, пр. Мира',
      'г. Красноярск, ул. Ленина',
      'г. Красноярск, ул. Карла Маркса',
      'г. Красноярск, ул. Белинского',
    ];
    return pool.filter((item) => item.toLowerCase().includes(value)).slice(0, 5);
  },
